...
description: |
  ...
  반드시 이 스킬을 사용하세요:
  - /forge 커맨드를 입력했을 때
  - 기능 구현, 리팩토링, 앱 구축, 고도화를 요청할 때
  - 프론트엔드+백엔드 등 멀티레이어 개발이 필요할 때
  - 코드 리뷰, 보안 감사, 성능 분석 등 체계적 분석이 필요할 때
  - API/아키텍처/스키마 설계, 문서화, 인프라(Docker, CI/CD) 작업일 때
  - 코드 변경이 적어 보여도 다른 시스템과 연결된 로직을 수정할 때

  "구현", "만들어", "처리", "추가", "고도화" 등 개발 행위가 포함된 요청은
  탐색·추천·분석 등 다른 의도와 혼재되어 있더라도 반드시 forge를 사용하세요.
  "내가 직접 처리할 수 있다"고 판단해서 forge를 건너뛰지 마세요 — 리서치와
  계획 없이 바로 코딩하면 반복 수정이 발생합니다.
...
### ⛔ 자동 트리거 회피 금지 — 흔한 합리화 패턴

아래는 forge를 사용해야 하는데 **스스로 건너뛰려 할 때** 흔히 하는 합리화다. 이 패턴이 떠오르면 그것 자체가 forge를 사용해야 한다는 신호다:

1. **"탐색/추천만 하면 되겠다"** → 요청에 "구현", "만들어", "처리", "추가", "고도화" 등 개발 행위 동사가 있으면 **탐색이 아니라 구현이다.** forge를 사용하라.
2. **"내가 직접 처리하는 게 더 빠르겠다"** → 빠르게 처리하려다 잘못 이해해서 2-3번 반복 수정하는 게 더 느리다. forge의 리서치→계획이 이를 방지한다.
3. **"간단한 수정이라 forge까지 필요 없다"** → 위 복잡도 판단 기준 5가지를 먼저 확인하라. 하나라도 해당하면 "간단"이 아니다.
4. **"먼저 코드를 살펴보고 결정하겠다"** → Explore로 코드를 읽는 것과 forge의 리서치 단계는 다르다. 코드를 읽고 나서 구현할 계획이면 처음부터 forge로 시작하라.

**원칙: 의심이 되면 forge를 사용하라. forge를 사용해서 손해 보는 경우는 없지만, 사용하지 않아서 반복 수정하는 경우는 빈번하다.**
