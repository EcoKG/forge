...
description: |
  ...
  - API/아키텍처/스키마 설계, 문서화, 인프라(Docker, CI/CD) 작업일 때

  코드 변경이 적어 보여도(단일 파일, 몇 줄) 해당 로직이 상태 흐름·
  구매/라이프사이클·UI 연동·설정 의존성 등 다른 시스템과 연결된 경우에도
  반드시 이 스킬을 사용하세요. 잘못 이해하고 수정하는 비용이 코딩 비용보다
  클 때 forge의 리서치→계획이 반복 수정을 방지합니다.
  오타 수정, 로그 추가 등 시스템 의존성 없는 독립 변경만 제외합니다.
